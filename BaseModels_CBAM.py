...
from keras.layers import Input, Conv2D, Add, BatchNormalization, Multiply, GlobalAveragePooling2D, GlobalMaxPooling2D, Reshape, Concatenate, Dropout, Dense
...
def cbam_block(input_tensor, ratio=8):
    ...
    avg_fc = shared_dense(avg_pool)
    max_fc = shared_dense(max_pool)
    channel_attention = Dense(channel, activation='sigmoid', use_bias=False)(Add()([avg_fc, max_fc]))
    ...
    spatial_attention = Conv2D(1, (7, 7), padding='same', activation='sigmoid')(Concatenate(axis=-1)([avg_pool, max_pool]))
    output = Multiply()([x, spatial_attention])
    return output

def insert_cbam(model_name, base_model_output):
    """根据模型类型插入一个深层 CBAM 模块"""
    if model_name == 'vgg16':
...
if args.model == 'vgg16':
    ...
    x = insert_cbam('vgg16', base_model)
    ...
    x = insert_cbam('densenet121', base_model)
    x = Dropout(0.5)(x)
    ...
    x = insert_cbam('resnet101', base_model)
    x = Dropout(0.5)(x)
    ...
    x = insert_cbam('mobilenetV3Large', base_model)
    x = Dropout(0.5)(x)
    ...
    x = insert_cbam('inceptionV3', base_model)
    x = Dropout(0.5)(x)
    ...
    x = insert_cbam('convnext_small', base_model)
    x = Dropout(0.5)(x)
    ...
    x = insert_cbam('convnext_base', base_model)
    x = Dropout(0.5)(x)
    ...
    x = insert_cbam('convnext_large', base_model)
    x = Dropout(0.5)(x)
